...
2. **CI runs static checks only.** AOSP builds are NOT run on GitHub Actions — they take 2-6 hours and would burn the free tier in a single build. The CI workflow validates scripts, markdown, secrets, and links. See [CI checks](ci-checks.md).
...
3. Update relevant docs:
   - **New tool / script**: add to the [tools reference](../reference/tools-reference.md) and the [architecture overview](../architecture/overview.md) if it introduces a new concept.
...
4. Run the local CI checks before pushing (see [CI checks](ci-checks.md)).
...
- Want to know what CI checks your PR against? → [CI checks](ci-checks.md)
- Want the design rules your PR must respect? → [Architecture overview](../architecture/overview.md)
